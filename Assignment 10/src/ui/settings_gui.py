...
class RadioButton:
    def __init__(self, x_position, y_position, button_text, buttons_group):
        pygame.init()

        self.rectangle_area = pygame.Rect(x_position, y_position, 20, 20)
        self.text = button_text
        self.selected = False
        self.group = buttons_group
...
class SettingsGUI:
    ...
    def request_board_size(self):
        for button in self.board_size_buttons:
            if button.is_selected():
                ...
                return int(board_size[ROWS]), int(board_size[COLUMNS])
    ...
    def want_to_play_first(self):
        return self.play_first_buttons[DEFAULT_SELECTION].is_selected()